...
namespace Azure.AI.Language.MCP.Server.Clients.Language
{
    ...
    public class LanguageClient : TextAnalysisClient, ILanguageClient
    {
        ...
        public async Task<AnalyzeTextResult> AnalyzeAsync(AnalyzeTextInput analyzeTextInput, bool? showStatistics = null, CancellationToken cancellationToken = default)
        {
            var response = await AnalyzeTextAsync(analyzeTextInput, showStatistics, cancellationToken);
        ...
        private async Task<Azure.Response> WaitForDocumentAnalysisCompletionAsync(string? operationLocation)
        {
            var cts = new CancellationTokenSource(_timeoutForJobs);
            while (true)
            {
                Azure.Response response = await GetResponseAsync(operationLocation, cts.Token);
        ...
        private async ValueTask<Azure.Response> GetResponseAsync(string uri, CancellationToken cancellationToken)
        {
